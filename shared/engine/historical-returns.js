...
export function statsForRange(startYear, endYear) {
    const rows = HISTORICAL_RETURNS.filter(r => r.year >= startYear && r.year <= endYear);
    if (!rows.length)
        return { meanReturn: 0.07, volReturn: 0.15, meanInflation: 0.025, volInflation: 0.02, yearsIncluded: 0 };
    const n = rows.length;
    const avg = (xs) => xs.reduce((a, b) => a + b, 0) / xs.length;
    const std = (xs, m) => Math.sqrt(xs.reduce((s, v) => s + (v - m) ** 2, 0) / xs.length);
 ...
 */
export function bootstrapYear(rand = Math.random) {
