...
export const PET_COST_CATEGORY_KEYS = ['petCare', 'petDaycare', 'petGrooming'];
...
export function buildPetCostByYear(pets, opts) {
    ...
    const curve = new Array(Math.max(0, years)).fill(0);
    if (!pets.length)
        return curve;
    for (let y = 0; y < years; y++) {
        ...
        for (const pet of pets) {
            if (calYear < pet.birthYear)
                continue; // not yet born / acquired
...
export function buildDependentCostByYear(dependents, opts) {
    ...
    const curve = new Array(Math.max(0, years)).fill(0);
    if (!dependents.length || !(monthlyCostPerDependent > 0))
        return curve;
    for (let y = 0; y < years; y++) {
        const calYear = simStartYear + y;
        let count = 0;
        for (const dep of dependents) {
            if (calYear < dep.birthYear)
                continue;
            const isAdult = dep.dependentType === 'adult';
            if (isAdult || calYear - dep.birthYear < untilAge)
                count++;
        }
