...
export const IRMAA_THRESHOLDS_2026 = {
...
export function irmaaTier(magi, filingStatus, indexFactor = 1) {
    const t = IRMAA_THRESHOLDS_2026[filingStatus];
    for (let i = 0; i < t.length; i++) {
        if (magi <= t[i] * indexFactor)
            return i;
    }
    return t.length;
}
/** Monthly surcharge per person for a tier, in 2026 dollars scaled by indexFactor. */
export function irmaaMonthlySurcharge(tier, includePartD = true, indexFactor = 1) {
