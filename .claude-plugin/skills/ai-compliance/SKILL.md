...
The regulation files in `compliance/regulations/` provide jurisdiction-specific guidance:
...
To answer jurisdiction-specific questions, read the relevant regional file from `compliance/regulations/`. Start with the Global Overview for cross-jurisdictional comparisons.

## Evidence Collection Pipeline

The pipeline produces filled compliance templates in `compliance/output/`. The workflow:

### 1. Configure: `compliance/tools/compliance-config.json`
...
### 2. Interactive Tools: `compliance/tools/interactive/*.html`
...
### 3. Auto-fill: `node compliance/tools/autofill.js`

Populates all 22 evidence templates from config data and interactive tool results:

```bash
node compliance/tools/autofill.js --config compliance/tools/compliance-config.json --output compliance/output/
```

### 4. Validate: `node compliance/tools/evidence-checker.js`

Checks all templates for completeness and compliance gaps:

```bash
node compliance/tools/evidence-checker.js --config compliance/tools/compliance-config.json --output compliance/output/
node compliance/tools/evidence-checker.js --config compliance/tools/compliance-config.json --output compliance/output/ --json
node compliance/tools/evidence-checker.js --config compliance/tools/compliance-config.json --output compliance/output/ --template 07
```
...
Hand the `compliance/output/` folder to a legal or compliance expert. The output includes:
...
- `compliance/tools/data/jurisdiction-matrix.json` — Which templates each jurisdiction requires
- `compliance/tools/data/deadline-data.json` — 21 compliance deadlines with dates and jurisdictions
